...
function getArtistByIndex(artistsArray, artistIndex) {
  const { id, name } = artistsArray[artistIndex]
...
function get20s(artistsArray){
  ...
  for (let i = 0; i < artistsArray.length; i++) {
    const { years } = artistsArray[i]
    const [birth, death] = years.split(' - ').map(s => parseInt(s))
    if (birth >= 1900 && death <= 2000) {
      resultArtists.push(artistsArray[i].name)
    }
  }
...
function removeArtist(artistsArray, removeIndex){
  artistsArray.splice(removeIndex, 1)
...
function addArtist(artistsArray){
  const me = {
    ...
    bio: "I'm an opera singer and a budding web developer. I live in NYC!"
  }
  
  artistsArray.push(me)
...
function lotsOfArt(artistsArray){
  ...
  for (let i = 0; i < artistsArray.length; i++) {
    const { paintings } = artistsArray[i]
    if (paintings > 100) {
      resultArtists.push(artistsArray[i].name)
    }
  }
